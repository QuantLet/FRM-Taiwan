  ...
  ### check which event occurs
    ...
    if (length(indV) == m){
      ...
      # for (j in 1:2) {
        ...
        tmpqr <- qr(tmpA1)
        condition_number <- kappa(tmpA1)
        if (condition_number > 1e4 || tmpqr$rank < tmplvar) {
          tmplam1 <- Inf
        } else {
          tmpu1 <- qr.solve(tmpqr, tmpb)
          if (any(is.na(tmpu1)) || any(abs(tmpu1) > 1e4) || tmpu1[tmplvar] <= 0) {
            tmplam1 <- Inf
          } else {
            tmpV <- c(indV, jstar)
        ...
        tmpqr <- qr(tmpA2)
        condition_number <- kappa(tmpA2)
        if (condition_number > 1e4 || tmpqr$rank < tmplvar) {
          tmplam2 <- Inf
        } else {
          tmpu2 <- qr.solve(tmpqr, tmpb)
          if (any(is.na(tmpu2)) || any(abs(tmpu2) > 1e4) || tmpu2[tmplvar] >= 0) {
            tmplam2 <- Inf
          } else {
            tmpV <- c(indV, jstar)
